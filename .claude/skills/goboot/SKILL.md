...
Plugins are compile-time (no dynamic loading). Implement `plugin.Plugin` plus any of `AnnotationProvider` (register annotations), `Analyzer` (diagnostics), `Generator` (files), `DialectProvider` (a DB driver's placeholder style). Standalone-module examples, one per capability: `plugins/oracle` (`DialectProvider` — Oracle dialect), `plugins/openapi` (`Generator` — OpenAPI 3 spec), `plugins/lint` (`Analyzer` — REST convention warnings); `plugin/exampleplugin` exercises all four in-module. The `Analyzer`/`Generator` hooks receive `model.Application`: structured routes/components **plus** `app.Declarations` (every annotated declaration with its raw annotations, including the plugin's own — filter via `app.DeclarationsWith("MyAnnotation")`), so an `AnnotationProvider` plugin can drive generation from its own annotations (§46.5). Host them by listing the module in `goboot.yaml` (`goboot generate` self-bootstraps a plugin-aware CLI), or by building a small `main` calling `cli.Main(pluginA.New(), ...)`. Full guide: `PLUGINS.md`.
